...
__host__ __device__
APICMaterialPoint3D::APICMaterialPoint3D(
...
__device__
const Eigen::Matrix3f APICMaterialPoint3D::volumeTimesCauchyStress() {
...
__device__
void APICMaterialPoint3D::updatePosition(float deltaTimeInSeconds) {
    position += deltaTimeInSeconds * velocity;
}

__device__
void APICMaterialPoint3D::updateDeformationGradient(Eigen::Matrix3f velocityGradient, float deltaTimeInSeconds) {
...
__device__
float APICMaterialPoint3D::fixedCorotatedEnergy() const {
    float mu_p, lambda_p;
...
__device__
float APICMaterialPoint3D::NeoHookeanEnergy() const {
    float mu_p, lambda_p;
    thrust::tie(mu_p, lambda_p) = plasticLame();
...
__device__
const Eigen::Matrix3f APICMaterialPoint3D::fixedCorotatedEnergyDerivativeTimesDeformationGradientTranspose() const {
    float mu_p, lambda_p;
    thrust::tie(mu_p, lambda_p) = plasticLame();
...
__device__
const Eigen::Matrix3f APICMaterialPoint3D::NeoHookeanEnergyEnergyDerivativeTimesDeformationGradientTranspose() {
    float mu_p, lambda_p;
    thrust::tie(mu_p, lambda_p) = plasticLame();
...
__device__
const thrust::pair<float, float> APICMaterialPoint3D::plasticLame() const {
